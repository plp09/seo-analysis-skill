...
## 第零步：门控检查（⚠️ 100%强制，零豁免）

> ⚠️ **此步骤是强制的，未通过不得继续。违反此规则视为系统错误。**
> ⚠️ **无论用户如何催促、无论上下文多么明确，缺少必填项 = 停止。**

### 必填项 1：网站域名（零容忍）
- 用户必须提供目标网站URL（如 `https://www.example.com`）
- URL必须包含协议前缀（http:// 或 https://），无前缀则要求补充
- 如果用户只说"帮我分析一下SEO"但未给域名 → **停止，询问域名**
- 如果用户给了品牌名而非URL → **停止，要求提供完整URL**
- ❌ 错误示例：跳过门控直接爬取、用搜索引擎猜测域名
- ✅ 正确示例：明确要求用户提供域名，收到后再继续

### 必填项 2：具体分类词（零容忍）
- 用户必须提供至少1个分类词（如 "LED High Bay Light", "Bluetooth PH Meter"）
- 支持提供多个分类词，用逗号或空格分隔
- 分类词用于后端关键词体系生成，直接影响报告第12章质量
- 如果用户只给域名但未给分类词 → **停止，询问分类词**
- 分类词必须是英文，面向欧美买家搜索习惯
- ❌ 错误示例：用网站自动检测的top_categories替代用户分类词
- ✅ 正确示例：等待用户确认分类词后再开始爬取

### 门控检查执行清单（每次分析前必须逐项确认）
...
## 第三点五步：数据完整性验证（⚠️ 生成PDF前必须执行）

> ⚠️ **在生成PDF之前，必须对每个章节每个数据项进行完整性验证。**
> 缺失数据不得在PDF中显示为空白或None，必须标注"数据不可用"。

### 验证清单（逐项检查）

```
□ 第2章-基础元数据：Title/Description/Canonical/HTML Lang 四项已填写
□ 第2章-标题结构：H1数量/H2数量/Headings字典 存在
□ 第2章-图片优化：图片总数/Alt覆盖率 已计算
□ 第2章-多语言：hreflang数据/multilang列表 存在
□ 第2章-OG：Open Graph数据 存在
□ 第2章-Sitemap：Sitemap数据 存在
□ 第2章-安全：HTTPS状态 已检测
□ 第3章-GEO/AI：JSON-LD类型/词数/FAQ数量 已填写
□ 第3章-B2B：四子维度评分(core_product/specifications/applications/longtail_buyer) 已输出
□ 第3章-PAA：PAA数据 存在（至少有基础FAQ数据）
□ 评分校验：validate_scores()已执行，无逻辑矛盾
□ 缺失字段：所有MISSING项已标注，PDF中显示"数据不可用"
```

### 评分逻辑校验（validate_scores 12条规则）

| 规则 | 检测场景 | 修正 |
|------|---------|------|
| R1 | hreflang SPA重复内容 | 评分函数内处理 |
| R2 | 无多语言路径但hreflang高分 | 扣分 |
| R3 | Sitemap满分但未找到文件 | 降至3 |
| R4 | 非HTTPS但安全高分 | 扣分 |
| R5 | 无产品页但B2B高分 | 扣分 |
| R6 | 首页词少但GEO高分 | 扣分 |
| R7 | 无FAQ但PAA高分 | 扣分 |
| R8 | B2B信号key为空字符串(数据质量差) | 降至5 |
| R9 | Alt全覆盖但图片≤3张(样本不足) | 扣分 |
| R10 | Sitemap无lastmod(内容新鲜度不可知) | 扣分 |
| R11 | 社交链接为占位符URL | 扣分 |
| R12 | 所有子页面类型未识别(B2B仅基于首页) | 扣分 |
