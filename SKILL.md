...
```
1. 确认目标 → 2. 爬取数据 → 3. 评分计算 → 4. AI生成关键词推荐 → 5. 生成HTML报告 → 6. 转PDF → 7. 发送PDF交付
```

> ⚠️ **PDF是最终交付物，不是可选项。** 每次分析必须以PDF文件发送给用户，HTML仅作为中间产物。
...
确认以下信息（缺少则询问用户）：
- 目标网站URL（1-2个）
- 网站所属**行业/产品类别**（重要！用于生成精准B2B关键词推荐）
- 目标市场区域（默认：欧美/全球）
- 报告标题（可选，默认"SEO Analysis Report"）

> ⚠️ **行业信息至关重要**：B2B外贸关键词推荐需要结合具体行业才能给出精准建议。如果用户未提供行业信息，爬取完成后需根据网站内容推断行业，并在报告中标注"行业为AI推断，建议确认"。
...
```bash
python3 "{SKILL_DIR}/scripts/seo_crawl.py" "https://TARGET.com" ["https://TARGET2.com"]
```
...
输出JSON到stdout，包含所有原始数据 + B2B关键词信号分析。
...
报告包含15个章节：
1. 综合评分（13维度评分表 + 综合得分）
2. 基础元数据（Title/Description/Keywords/Canonical）
3. 标题结构（H1/H2统计）
4. 图片优化（Alt覆盖率）
5. 多语言与国际化（hreflang验证 — **扫描全页面源码**，不仅限head区域）
6. Open Graph与社交分享（OG标签 + **社交平台入口检测**：WhatsApp/Facebook/YouTube/WeChat/LinkedIn）
7. Sitemap质量
8. 技术安全与清洁度
9. 内容体量评估（词数统计）
10. GEO/AI结构化数据（**仅检测4种**：FAQPage/HowTo/Organization/NewsArticle）
11. GEO/AI内容质量（FAQ/HowTo区块 + 词数）
12. **B2B外贸买家关键词覆盖**（四大子维度信号检测 + 评分 + **基于网站Top3分类词推荐核心产品词**）
13. **页面级 PAA 内容检测**（FAQPage schema + 手风琴组件 + 疑问式标题 + PAA示例生成）
14. 综合结论（优势与待改进维度）
15. 修复优先级Action Plan（P0/P1/P2分级）
...
使用 `message` 工具将PDF文件发送给用户：

```
message action=send target=用户 filePath=/path/to/report.pdf message="SEO分析报告PDF已生成"
```

或使用 `MEDIA:` 指令在回复中附带PDF文件：

```
MEDIA:/path/to/report.pdf
```

> ⚠️ **每次分析结束前，必须确认PDF已成功发送/交付给用户。** 如果发送失败，提示用户文件路径并说明可手动下载。
...
| 社交分享优化 | 0.7 | 0-10 | |
| 技术安全性 | 1.2 | 0-10 | |
| GEO/AI结构化数据 | 1.3 | 0-10 | |
| GEO/AI内容质量 | 1.3 | 0-10 | |
| **B2B外贸关键词覆盖** | **1.4** | **0-10** | **含4个子维度** |

### 维度12: B2B外贸关键词覆盖 — 4个子维度
